...
var CELULAR_EMPRESA = '5513997546382';
...
aumentarQuantidade: (id) => {
  ...
  carregarCarrinho: () => {
   ...
   atualizarCarrinho: (id, qntd) => {
    ...
    // atualiza os valors (R$) totais do carrinho
    cardapio.metodos.carregarValores();
   ...
   carregarValores: () => {
       ...
       $.each(MEU_CARRINHO, (i, e) => {
         ...
         if ((i + 1)  == MEU_CARRINHO.length) {
           ...
           $("#ValorEntrega").text(`+ R$ ${VALOR_CARRINHO.toFixed(2).replace('.', ',')}`);
  ...
  buscarCep: () => {
      ...
      if (cep != "") {
        ...
        if (validacep.test(cep)) {
         ...
         cardapio.metodos.mensagem('CEP não encontrado. Preencha as infrmações manualmente.');
            $("#txtEndereco").focus();
        }
    ...
    // validaça2o antes de prosseguir para a etapa 3
  resumoPedido: () => {
